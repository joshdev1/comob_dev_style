...
def train_network(w1, w2, training_data, ipds, spikes, loss_fn, log_softmax_fn, optimizer):
    loss_hist = []
    for e in range(nb_epochs):
        local_loss = []
        for x_local, y_local in data_generator(training_data.discretise(ipds), spikes):
            output = run_network(x_local, w1, w2)
            loss = _cross_entropy(output, loss_fn, log_softmax_fn, y_local, local_loss)
...
def _cross_entropy(output, loss_fn, log_softmax_fn, y_local, local_loss):
